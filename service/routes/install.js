...
router.route('/')
  .get(function (req, res, next) {
    let { page, timeRange, customerId, installNumber } = req.query;
    ...
    if (installNumber) {
      queryCondition['installNumber'] = new RegExp(installNumber);
    }
    Install.count(queryCondition, function (err, count) {
      Install.find(queryCondition)
        .limit(limit)
        .skip(skip)
            ...
            if (count > 0) {
              ...
              Customer.find({}, function (err, customers) {
                ...
                installs.map((install) =>
                  install['customerName'] = customerMap[order['customerId']]
                );
                res.send({
                  success: true,
                  installs: installs,
